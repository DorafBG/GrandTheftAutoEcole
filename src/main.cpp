...
int main()
{
    ...
    //ajout de yellow house
    Shader* model_shader = new Shader(shader_dir + "texture.vert", shader_dir + "texture.frag");
    Model* yellow_house = new Model("/home/gegestreetfight/Documents/Travail/InformatiqueGraphique/blenderImages/yellowHouse2.obj",model_shader);

    // Création d'une matrice de transformation appropriée au lieu de glm::mat4(1.0f)
glm::mat4 model_transform = glm::mat4(1.0f);  // Commencer par une matrice identité

    // Translation pour positionner le modèle dans l'espace
    // Paramètres : (x, y, z) - déplacer vers l'avant (z négatif), et peut-être un peu vers le bas (y négatif)
    model_transform = glm::translate(model_transform, glm::vec3(0.0f, 3.0f, -5.0f));

    // Rotation si nécessaire (par exemple, pour orienter correctement le modèle)
    // Paramètres : angle en radians, axe de rotation
    model_transform = glm::rotate(model_transform, glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    // Mise à l'échelle si le modèle est trop grand ou trop petit
    // Paramètres : facteurs d'échelle (x, y, z)
    model_transform = glm::scale(model_transform, glm::vec3(0.5f, 0.5f, 0.5f));

    // Créer le nœud avec cette matrice de transformation
    Node* model_node = new Node(model_transform);
    model_node->add(yellow_house);
    viewer.scene_root->add(model_node);
